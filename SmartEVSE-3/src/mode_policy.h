 ...
 * the Smart operating mode from the web UI or LCD menu.
 * Normal mode can never be disabled — it is the safety fallback.
 *
 * Also provides the LCD status text ("SMART PAUSED", "NORMAL", ...) so the
...
#define MODE_DISABLE_ALL     (MODE_DISABLE_SMART)
 ...
 * Produces "NORMAL", "SMART" with " OFF" / " PAUSED" appended
